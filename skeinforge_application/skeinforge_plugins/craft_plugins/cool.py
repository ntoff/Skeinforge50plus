...
class CoolRepository:
	...
	def __init__(self):
		...
		self.fanmaxPWM = settings.IntSpin().getFromValue(0, 'Full Fan Speed (%):', self, 100, 100)
		...
		for self.lineIndex in xrange(len(self.lines)):
			...
			if firstWord == 'M108':
				...
				if self.repository.turnFanOnAtBeginning.value:
					self.distanceFeedRate.addLine('M106 S%d' % (round(255*(self.repository.fanmaxPWM.value/100))) )
		...
		if firstWord == 'G1':
			...
			if not self.repository.turnFanOnAtBeginning.value and self.layerCount.layerIndex >= self.repository.fanFirstLayer.value:
				if (self.repository.bridgeFan.value and self.isBridgeLayer) or layerTime <= self.repository.fullFanLayerTime.value:
					...
					fanPWM = round( ( ( self.repository.maximumFanLayerTime.value - layerTime ) / fanPWMArea ) * (round(255*(self.repository.fanmaxPWM.value/100))) )
