...
class SpeedRepository:
	...
	def __init__(self):
		...
		self.flowRateSetting = settings.FloatSpin().getFromValue( 0.0, 'Flow Rate Multiplier (ratio):', self, 1.0, 1.0 )
		settings.LabelSeparator().getFromRepository(self)
		...
		self.infillFeedRateMultiplier = settings.FloatSpin().getFromValue(0.5, 'Infill Feed Rate Multiplier (ratio):', self, 1.0, 1.0)
		self.infillFlowRateMultiplier = settings.FloatSpin().getFromValue( 0.5, 'Infill Flow Rate Multiplier (ratio):', self, 1.0, 1.0 )
		settings.LabelSeparator().getFromRepository(self)
...
class SpeedSkein:
	...
	def getSpeededLine(self, line, splitLine):
		...
		if self.isEdgePath:
			...
			feedRateMinute *= self.repository.infillFeedRateMultiplier.value
		if self.layerIndex == 0:
			if self.isEdgePath:
