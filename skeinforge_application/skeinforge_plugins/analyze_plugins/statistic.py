...
class StatisticRepository:
	...
	def __init__(self):
		...
		settings.LabelSeparator().getFromRepository(self)
		settings.LabelDisplay().getFromName('- Calibration -', self )
		self.timeAdj = settings.FloatSpin().getFromValue( 0.1, 'Time Adjust (ratio):', self, 2.0, 1.0 )
...
class StatisticSkein:
	...
	def addToPath(self, location):
		...
		if self.oldLocation != None:
			...
			if self.feedRateMinute > 0.0:
				self.totalBuildTime += 60.0 * travel / self.feedRateMinute * self.timeAdjuster
	...
	def getCraftedGcode(self, gcodeText, repository):
		...
		self.timeAdjuster = repository.timeAdj.value
		self.absoluteEdgeWidth = 0.0
		self.characters = 0
